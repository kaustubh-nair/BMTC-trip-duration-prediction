...
big_df = pd.read_csv('../dataset/small.csv', dtype=dtypes, chunksize=1000000)
...
for df in big_df:
    for index, row in df.iterrows():
        if row['BusID'] not in bus:
            print(row['BusID'])
            bus.append(row['BusID'])
...
#   folium.CircleMarker(location=[row['LAT'], row['LON']], color=time[row['Timestamp'].split(" ")[0]], radius=2, weight=2).add_to(mapit)
    #i += 1
#mapit.save('map.html')
